document.title="Maiores despesas por grupos";
...
$("#processar_grafico").on('click',function(){
	...
	data_table.addColumn('string', 'Despesa');
	data_table.addColumn('number', 'Valor');
	...
	$.get('top_despesas_grupos_graf?conta='+$("#busca_contas").val()+'&data_ini='+$("#data_ini").val()+'&data_fim='+$("#data_fim").val(), function (data) {
	...
	var chart = new google.visualization.PieChart(document.getElementById('top_gastos_grupos'));
	...
	function selectHandler() {
			...
			if (selectedItem) {
				...
				$.get('../graficos/top_despesas_grupos_det?id='+id+'&conta='+$("#busca_contas").val()+'&data_ini='+$("#data_ini").val()+'&data_fim='+$("#data_fim").val(), function (data) {
					//alert(data);
					$("#table_modal thead").html("");
					$('#table_modal tbody').html("");
					$('#detalhesGruposModalLabel').html("");
					$('#detalhesGruposModalLabel').html("Lançamentos do grupo "+  grupo);
					...
					$("#detalhesGruposModal").modal('show');
